...
while True:
    ...
    if keyboard.key_pressed("D") and topPad.x < window.width - topPad.width:
        topPad.x = topPad.x + paddingVelocity * window.delta_time()
    if keyboard.key_pressed("A") and topPad.x > 0:
        topPad.x = topPad.x - paddingVelocity * window.delta_time()
    ...
    background.draw()
    window.draw_text(f"{scoreTop}", 20, 30, 30, (0, 0, 0))
    window.draw_text(f"{scoreBottom}", 20, window.height - 60, 30)
    ball.draw()
    bottomPad.draw()
    topPad.draw()
    window.update()
